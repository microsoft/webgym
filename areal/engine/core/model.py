...
VALID_VISION_MODELS = [
    "qwen2_vl",
    "qwen2_5_vl",
    # Some HF revisions of Qwen2.5-VL-based checkpoints (e.g. microsoft/Fara-7B)
    # report `model_type` as ``qwen2_5_vl_text`` even though the architecture
    # is the full multimodal Qwen2_5_VLForConditionalGeneration. Treat it the
    # same as ``qwen2_5_vl`` so the VLM code paths (from_pretrained via
    # AutoModelForImageTextToText, image-token handling, ulysses patching)
...
def is_qwen2_vl_model(model_type: str) -> bool:
    return model_type in ["qwen2_vl", "qwen2_5_vl", "qwen2_5_vl_text"]
...
def is_qwen3_moe_model(model_type: str) -> bool:
    return model_type in ["qwen3_moe", "qwen3_vl_moe"]
