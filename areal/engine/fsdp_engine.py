...
# Process-level pixel cache shared between colocated engines (actor + ref).
# This avoids ref workers re-fetching images already fetched by actor workers.
_PROCESS_PIXEL_CACHE: dict[str, torch.Tensor] | None = None


def _get_process_pixel_cache() -> dict[str, torch.Tensor]:
    global _PROCESS_PIXEL_CACHE
    if _PROCESS_PIXEL_CACHE is None:
        _PROCESS_PIXEL_CACHE = {}
    return _PROCESS_PIXEL_CACHE
...
class FSDPEngine(TrainEngine):
    def __init__(self, config: TrainEngineConfig):
        ...
        # Cache for pixel tensors fetched from Ray object store via broker.
        # Process-level cache shared between colocated engines (actor + ref)
        # so ref workers reuse images already fetched by actor workers.
        self._pixel_ref_cache = _get_process_pixel_cache()
        self._pixel_store = None  # Lazy-initialized PixelRefStore handle
        self._last_pixel_prefetch_secs: float = 0.0  # Last call's prefetch time
        self._cumul_pixel_prefetch_secs: float = 0.0  # Accumulator (reset by trainer)
    ...
    def initialize(self, addr: str | None, ft_spec: FinetuneSpec, *args, **kwargs):
        ...
        print(f"[FSDPEngine] PYTORCH_ALLOC_CONF={_alloc_conf} (pid={os.getpid()})")
    ...
    def forward_backward_batch(
        ...
        for _mb_idx, mb_item in enumerate(mb_list):
            inputs, ctx = self._prepare_mb_inputs(mb_item)

            # Stream pixel_values to GPU per-microbatch (lazy transfer).
            # When stream_pixel_values=True in .to(), pixel_values stay on CPU
            # during the bulk transfer. Move them to GPU here, one microbatch
            # at a time, to avoid holding all images on GPU simultaneously.
            _pv_on_gpu = False
            if "pixel_values" in inputs and inputs["pixel_values"] is not None:
                pv = inputs["pixel_values"]
                if torch.is_tensor(pv) and not pv.is_cuda:
            ...
            # Release per-microbatch pixel_values from GPU after forward+backward.
            # Must be after backward in case gradient checkpointing recomputes
            # the vision encoder during the backward pass.
            if _pv_on_gpu:
    ...
    def train_batch(
        ...
        mb_list = self._prepare_mb_list(input_).to(
            self.device, stream_pixel_values=self.is_vision_model
        )
    ...
    def clear_pixel_cache(self) -> None:
        """Clear the process-level pixel cache between training steps."""
        self._pixel_ref_cache.clear()
    ...
    def _load_critic_head_from_checkpoint(self, model, path: str, dtype):
        """Load trained MLP value head weights from a critic checkpoint."""
        ...
        ckpt_path = Path(path)
        head_state = {}
        for sf_file in ckpt_path.glob("*.safetensors"):
            with safe_open(str(sf_file), framework="pt", device="cpu") as f:
                for key in f.keys():
                    if key.startswith("lm_head."):
                        stripped_key = key[len("lm_head."):]
                        head_state[stripped_key] = f.get_tensor(key)

        if not head_state:
            self.logger.info("VLM critic: no MLP head weights in checkpoint, using random init")
            return

        mlp_keys = {k: v for k, v in head_state.items() if "." in k}
        if not mlp_keys:
            self.logger.info("VLM critic: checkpoint has base lm_head, not MLP — using random init")
            return

        missing, unexpected = model.lm_head.load_state_dict(mlp_keys, strict=False)
        if missing:
            self.logger.warning("VLM critic head: missing keys: %s", missing)
        if unexpected:
            self.logger.info("VLM critic head: ignored keys: %s", unexpected)
        if not missing and not unexpected:
            self.logger.info(
                "VLM critic: loaded trained MLP head from checkpoint (%d tensors)",
                len(mlp_keys),
            )
    ...
    def _create_device_model(self):
        ...
        if self.is_vision_model:
            ...
            with torch.device(loading_device):
                if self.config.is_critic:
                    # VLM critic: use Qwen3VLForCritic which has MLP value
                    # head and disabled weight tying (no tie_word_embeddings).
                    from areal.models.transformers.qwen3_vl import Qwen3VLForCritic

                    model = Qwen3VLForCritic.from_pretrained(
                        pretrained_model_name_or_path=self.config.path,
                        trust_remote_code=True,
                        dtype=dtype,
                        attn_implementation=self.config.attn_impl,
                    ...
                    self.logger.info(
                        "VLM critic: Qwen3VLForCritic with MLP value head "
                        "(Linear %d → 256 → 256 → 1), tie_word_embeddings=False",
    ...
    def _create_optimizer(self, ft_spec: FinetuneSpec) -> None:
        ...
        total_train_steps = (
            self.optimizer_config.lr_scheduler_max_steps
            if self.optimizer_config.lr_scheduler_max_steps is not None
            else ft_spec.total_train_steps
        )
    ...
    def _get_model_name_parameters(self) -> Iterator[tuple[str, nn.Parameter]]:
        # Yield HF-native parameter names so that SGLang's load_weights()
        # can map them to its internal structure (e.g. stacking q/k/v into
        # qkv_proj, stripping "model.language_model." to "model.", etc.).
    ...
    def _save_model_to_hf(
        ...
        if dist.get_rank() == 0:
            ...
            # Write completion marker so test_rollout can distinguish
            # a fully-written checkpoint from one still being saved.
            _marker = os.path.join(path, ".save_complete")
    ...
    def _load_from_dcp(self, path: str, with_optim: bool):
        ...
        LR scheduler state is intentionally NOT serialized — the caller is
        responsible for replaying ``lr_scheduler.step()`` the right number of
        times after load (see RecoverHandler) so the current lambda is the one
        in effect. Persisting a serialized ``_last_lr`` led to drift between
        the saved scheduler value and the LR the original run actually used.
        """
        if self.model is None:
            raise RuntimeError("Model not initialized")
    ...
    def catch_up_lr_scheduler(self, n_steps: int) -> None:
        """Advance the LR scheduler ``n_steps`` times.

        Used during resume: after DCP restores model + optimizer, the scheduler
        is still at its post-init last_epoch=0; this replay reconstructs the
        intended (last_epoch, lr) pair using the *current* schedule lambda.
        """
        if self.lr_scheduler is None or n_steps <= 0:
            return
        for _ in range(n_steps):
            self.lr_scheduler.step()
        sched_lr = self.lr_scheduler.get_last_lr()[0]
        optim_lr = self.optimizer.param_groups[0]["lr"]
        if abs(sched_lr - optim_lr) > 1e-12:
            raise RuntimeError(
                f"LR scheduler / optimizer disagree after catch-up: "
                f"scheduler.get_last_lr()={sched_lr}, "
                f"optimizer.param_groups[0]['lr']={optim_lr}"
            )
        self.logger.info(
            "LR scheduler caught up: last_epoch=%d, lr=%.6g",
    ...
    def _prepare_mb_list(self, input_: dict[str, Any]) -> MicroBatchList:
        ...
        self._last_pixel_prefetch_secs = 0.0
        input_ = input_.copy()
        ...
        if is_qwen_vl_model(self.model_config.model_type):
            ...
            # Qwen3-VL get_rope_index creates position_ids with input_ids.dtype
            # but internally uses int64 for llm_positions, causing dtype mismatch.
        ...
        # --- Batch prefetch all unique images before the microbatch loop ---
        # Parse store_keys once (same JSON replicated across all microbatches)
        _first_keys_json = (
            mb_list.mbs[0].get("_image_store_keys_json") if mb_list.mbs else None
        )
        if _first_keys_json:
            ...
            _t_prefetch_start = _pv_time.monotonic()
            _prefetch_store_keys = _json.loads(_first_keys_json)
            if self._pixel_store is None:
                self._pixel_store = get_or_create_pixel_store()

            # Collect ALL unique uncached keys across ALL microbatches
            _all_needed: list[str] = []
            _seen: set[str] = set()
            for _mb in mb_list.mbs:
                for _item in _mb.get("multi_modal_input", []):
                    if "image_indices" in _item:
                        for _idx in _item["image_indices"]:
                            _k = _prefetch_store_keys[_idx]
                            if _k not in self._pixel_ref_cache and _k not in _seen:
                                _all_needed.append(_k)
                                _seen.add(_k)

            if _all_needed:
                # Check for controller-prefetched images (one plasma object).
                # All workers share the same ref — zero-copy on same node,
                # one transfer for cross-node (then plasma-cached).
                _prefetched_ref = mb_list.mbs[0].get("_prefetched_pixels_ref")
                if _prefetched_ref is not None:
                    _all_images = ray.get(_prefetched_ref)
                    for _k in _all_needed:
                        if _k in _all_images:
                            _v = _all_images[_k]
                            # Controller stores as numpy for zero-copy Arrow IPC.
                            # Convert back to tensor (shares memory via as_tensor).
                            if not isinstance(_v, torch.Tensor):
                                _v = torch.as_tensor(_v)
                            self._pixel_ref_cache[_k] = _v
                    del _all_images
                else:
                    # Fetch from PixelRefStore actor (images stored as numpy)
                    _fetched = ray.get(self._pixel_store.get_slice.remote(_all_needed))
            ...
            self._last_pixel_prefetch_secs = _pv_time.monotonic() - _t_prefetch_start
            self._cumul_pixel_prefetch_secs += self._last_pixel_prefetch_secs
        ...
        for mb, padded_mb in zip(mb_list.mbs, mb_list.padded_mbs):
            ...
            if "multi_modal_input" in mb:
                ...
                # Build pixel_values: fetch from Ray object store via the
                # PixelRefStore broker.  Keys are JSON-encoded to avoid
                # DP dispatch splitting.  Worker-level cache ensures each
                # image is ray.get() only once per step.
                import time as _pv_time

                _t_pv_start = _pv_time.monotonic()
                store_keys = None
                _keys_json = mb.get("_image_store_keys_json")
                if _keys_json:
                    import json as _json

                    store_keys = _json.loads(_keys_json)
                image_store = mb.get("_image_store")
                pixel_values_list = []
                _ray_get_count = 0
                _cache_hit_count = 0
                for item in mb["multi_modal_input"]:
                    if "image_indices" in item:
                        if store_keys:
                            import ray
                            ...
                            # Collect uncached keys for this item
                            needed_keys = []
                            needed_indices = []
                            for idx in item["image_indices"]:
                                k = store_keys[idx]
                                if k not in self._pixel_ref_cache:
                                    needed_keys.append(k)
                                    needed_indices.append(idx)
                                else:
                                    _cache_hit_count += 1

                            # Fetch uncached images from broker
                            if needed_keys:
                                _fetched = ray.get(
                                    self._pixel_store.get_slice.remote(needed_keys)
                                )
                                self._pixel_ref_cache.update(_fetched)
                                del _fetched
                                _ray_get_count += len(needed_keys)

                            for idx in item["image_indices"]:
                                pixel_values_list.append(
                                    self._pixel_ref_cache[store_keys[idx]]
                                )
                        elif image_store is not None:
                            pixel_values_list.extend(
                                image_store[idx] for idx in item["image_indices"]
                            )
                    elif "pixel_values" in item:
                        pixel_values_list.append(item["pixel_values"])
                if pixel_values_list:
                    cat_pv = torch.cat(pixel_values_list, dim=0)
                    mb["pixel_values"] = cat_pv
                    padded_mb["pixel_values"] = cat_pv
                video_grid_thw_list = [
                    item["video_grid_thw"]
                    for item in mb["multi_modal_input"]
