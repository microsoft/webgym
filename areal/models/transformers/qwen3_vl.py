...
# ---------------------------------------------------------------------------
# Qwen3VLForCritic: VLM critic with MLP value head replacing lm_head.
# This is analogous to AutoModelForTokenClassification but for VLM models
# which don't have a native classification variant.
# ---------------------------------------------------------------------------
...
class Qwen3VLForCritic(Qwen3VLForConditionalGeneration):
    """Qwen3-VL model with MLP value head for critic (RL value function).

    Replaces lm_head (vocab projection) with a 3-layer MLP that outputs
    scalar value predictions. Weight tying is disabled so the MLP head
    is saved/loaded independently from the embedding layer.
    """

    # Override class-level tied weights — critic has no tied weights.
    _tied_weights_keys = []

    def __init__(self, config):
        super().__init__(config)
        # Disable weight tying at config level
        config.tie_word_embeddings = False
        self.config.tie_word_embeddings = False

        # Replace lm_head with MLP value head
        ...
        self.lm_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(mlp_hidden, mlp_hidden, bias=True),
            nn.ReLU(),
            nn.Linear(mlp_hidden, 1, bias=False),
        )
        # Post-init already called by super().__init__, re-init MLP head
        self.lm_head.apply(self._init_weights_mlp)

    @staticmethod
    def _init_weights_mlp(module):
    ...
    def tie_weights(self):
        """No-op: critic MLP head must not be tied to embeddings."""
        pass
