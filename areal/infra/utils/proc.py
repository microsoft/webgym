...
# Used to defang values that get spliced into the bash pipeline below — env-var
# names must look like real identifiers (used unquoted as ``KEY=value`` prefix),
# and the role tag is interpolated into a ``sed 's/^/{tag}/'`` expression so it
# must not contain sed-special characters.
...
def build_streaming_log_cmd(
    ...
    *,
    ...
    """
    # When cmd is a list every argv is shlex-quoted, eliminating injection
    # through any element. The string form is left as-is because the slurm
    # scheduler legitimately relies on bash to expand things like ``$PWD`` in
    # ``srun ... --chdir $PWD``. Callers must therefore only pass strings they
    # constructed themselves from trusted sources — never untrusted input.
    ...
    # Role goes into a sed expression — strip anything that would break out.
    safe_role = _ROLE_SAFE_RE.sub("_", role)
    log_prefix = f"[{safe_role}]".ljust(LOG_PREFIX_WIDTH)

    # Construct tee/sed pipeline; quote both log paths so a path containing
    # spaces / metacharacters can't change the shape of the command.
    shell_cmd = (
        f"{full_cmd} 2>&1 "
        f"| tee -a {shlex.quote(str(log_file))} "
        f">(stdbuf -oL sed 's/^/{log_prefix}/' >> {shlex.quote(str(merged_log))})"
    )
    return shell_cmd
