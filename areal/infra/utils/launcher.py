...
def get_env_vars(additional_env_vars: str | None = None) -> dict[str, str]:
    ...
    env = {**BASE_ENVIRONS, "PYTHONPATH": dynamic_pythonpath, **_additional_env_vars}

    # Propagate secrets from the parent process so Ray workers on remote nodes
    # can access external services (e.g., CPU cluster for browser instances).
    for key in ("CPU_CLUSTER_TOKEN", "HF_TOKEN", "WANDB_API_KEY", "PYTORCH_ALLOC_CONF"):
