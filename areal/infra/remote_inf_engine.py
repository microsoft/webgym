...
class GroupedRolloutWorkflow(RolloutWorkflow):
    ...
    async def arun_episode(
        ...
        # Launch all episodes concurrently (no stagger delay).
        tasks = [
            asyncio.ensure_future(
                self.workflow.arun_episode(engine, dict(data))
            )
            for _ in range(self.group_size)
        ]
        results = await asyncio.gather(*tasks)

        # Per-group collection failure counters (mirror test_rollout's
        # n_failed). One emission per group call -> wandb shows the
        # average failed episodes per group across the current stat
        # window. With group_size=8, "n_failed=0.4" means ~5% of
        # individual episode collections returned None (browser pool
        # 503, 401 mid-trajectory, max_retries exhausted, etc.).
        # Emitted before the early-return so all-fail groups still
        # count toward the failure rate.
        _n_total = len(results)
        _n_failed = sum(1 for r in results if r is None)
        if _n_total > 0:
            from areal.infra.workflow_context import stat_scope
            from areal.utils import stats_tracker
            stats_tracker.get(stat_scope()).scalar(
                **{
                    "trajectory/n_failed": float(_n_failed),
                    "trajectory/episode_failure_rate": float(_n_failed) / _n_total,
                }
            )
        ...
        # Stamp all episodes in this group with the same group_id so GRPO
        # advantage normalization groups by task, not by position.
        import math
        import torch

        group_id = id(results) % (2**31)  # unique per task invocation
        for r in valid_results:
            if isinstance(r, dict) and "episode_id" in r:
                T = r["episode_id"].shape[0]
                r["group_id"] = torch.full((T,), group_id, dtype=torch.int64)

        # Log pass@k and pass@(k//2) metrics on the rollout side.
        # Each valid_result is one episode with a "rewards" tensor.
        # Extract per-episode binary reward and compute group-level pass rates.
        if self.group_size > 1:
            ...
            for r in valid_results:
                if isinstance(r, dict) and "rewards" in r:
                    rval = r["rewards"]
                    ep_rewards.append(float(rval.float().max().item()))
            n = len(ep_rewards)
            c = sum(1 for r in ep_rewards if r > 0)
            self.logger.info(
                "[GRPO group] n_valid=%d/%d, rewards=%s, success=%d/%d",
                len(valid_results), self.group_size,
                [round(r, 2) for r in ep_rewards],
                c, n,
            )

            if n > 0:
                scope = stat_scope()
                # pass@k: at least 1 correct in all n episodes
                pass_k = 1.0 if c > 0 else 0.0

                # pass@(k//2): sample k//2 from n, P(at least 1 correct)
                # = 1 - C(n-c, k//2) / C(n, k//2)
                half_k = max(1, self.group_size // 2)
                if c >= n or half_k > n:
                    pass_k_half = 1.0 if c > 0 else 0.0
                else:
                    try:
                        pass_k_half = 1.0 - math.comb(n - c, half_k) / math.comb(n, half_k)
                    except (ValueError, ZeroDivisionError):
                        pass_k_half = 1.0 if c > 0 else 0.0

                # Group reward variance: 0 means all same (no GRPO signal)
                reward_var = sum((r - c / n) ** 2 for r in ep_rewards) / n
                zero_var = 1.0 if reward_var == 0.0 else 0.0

                stats_tracker.get(scope).scalar(
                    pass_at_k=pass_k,
                    pass_at_k_half=pass_k_half,
                    grpo_group_reward_var=reward_var,
                    grpo_zero_var_group=zero_var,
                    grpo_all_pass_group=1.0 if c == n else 0.0,
                    grpo_all_fail_group=1.0 if c == 0 else 0.0,
                )

                # Drop zero-variance groups (all-pass or all-fail). After
                # GRPO z-score normalization their advantages collapse to 0
                # and produce only KL-driven gradient — wasted compute. The
                # workflow_executor counts a None return as "rejected" so
                # the consumer keeps requesting until enough useful groups
                # arrive.
                if reward_var == 0.0:
                    # Free PixelRefStore images for the dropped group's
                    # n episodes. workflow_executor's outer reject-free
                    # only sees None and cannot reach into per-episode
                    # _image_store_keys, so without this free the images
                    # accumulate (~23% retention vs FBC's 0.26% observed
                    # 2026-04-26: 42933 stored / 184673 total puts on a
                    # GRPO run that OOM'd at 1.3TB master memory).
                    _all_keys: list[str] = []
                    for r in valid_results:
                        if isinstance(r, dict):
                            _ks = r.get("_image_store_keys")
                            if _ks:
                                _all_keys.extend(_ks)
                    if _all_keys:
                        try:
                            ...
                            ray.get(_store.free.remote(_all_keys))
                        except Exception:
                            pass  # best-effort cleanup
                    self.logger.info(
                        "[GRPO group] dropped zero-var group "
                        "(all_pass=%s all_fail=%s, c=%d/n=%d, freed=%d imgs)",
                        c == n, c == 0, c, n, len(_all_keys),
                    )
                    return None
...
class RemoteInfEngine(InferenceEngine):
    ...
    def __init__(
        ...
        # Default logger until initialize() sets a rank-specific one.
        # launch_server may be called before initialize().
        self.logger = logger
    ...
    def initialize(
        ...
        num_rollout_workers : int | None
            Total number of rollout workers. Passed to workflows for
            per-worker concurrency limiting and monitor port allocation.
        """
        self.num_rollout_workers = num_rollout_workers or 1
    ...
    def set_version(self, version):
        ...
        logger.info(
            "RemoteInfEngine set_version: %d -> %d", old_version, version
        )
    ...
    def _inject_worker_metadata(
        self, workflow_kwargs: dict[str, Any] | None
    ) -> None:
        """Inject per-worker metadata into workflow_kwargs env_config.

        Adds worker_rank, num_rollout_workers, and is_head_node so workflows
        can set up per-worker monitor ports, episode concurrency limits, and
        node-aware scheduling (e.g., reduced concurrency on the head node).
        """
        if workflow_kwargs is None:
            return
        env_config = workflow_kwargs.get("env_config")
        if not isinstance(env_config, dict):
            return
        env_config["worker_rank"] = int(self.engine_id) if self.engine_id else 0
        env_config["num_rollout_workers"] = getattr(
            self, "num_rollout_workers", 1
        )
        # Detect whether this worker runs on the Ray head node so workflows
        # can reduce concurrency on the head (which also runs the trainer,
        # Ray GCS, weight-sync coordinator, etc.).
        try:
            import ray
            ...
            head_nodes = [
                ...
                for n in ray.nodes()
                if n.get("Alive")
                and "node:__internal_head__" in n.get("Resources", {})
            ]
            env_config["is_head_node"] = (
                node_id == head_nodes[0]["NodeID"] if head_nodes else False
            )
        except Exception:
            env_config["is_head_node"] = False
    ...
    def _resolve_workflow(
        ...
        # Inject per-worker metadata before instantiation
        self._inject_worker_metadata(workflow_kwargs)

        # Cache key: reuse the same workflow instance for repeated calls with
        # the same workflow class/string.  This is critical for stateful
        # workflows (e.g. WebGymWorkflow) that hold per-worker monitors and
        # semaphores — recreating them on every submit() would break them.
        cache_key: str | None = None
        if isinstance(workflow, str):
            cache_key = workflow
        elif isinstance(workflow, type):
            cache_key = f"{workflow.__module__}.{workflow.__qualname__}"

        if cache_key is not None:
            cached = getattr(self, "_workflow_cache", {}).get(cache_key)
        ...
        # Cache for reuse on subsequent submit() calls
        if cache_key is not None:
    ...
    async def agenerate(self, req: ModelRequest) -> ModelResponse:
        ...
        # Use strict < to avoid SGLang boundary rejection (input + output must be < context_length)
        max_new_tokens = min(
            gconfig.max_tokens - len(req.input_ids) - 1, gconfig.max_new_tokens
        )
        if max_new_tokens <= 0:
            raise RuntimeError(
        ...
        while (
            ...
            if self.workflow_executor.is_paused():
                ver_before = self.get_version()
                self.logger.info(
                    "[ABORT] Generation paused during agenerate, "
                    "accumulated=%d tokens, version_before=%d, rid=%s",
                ...
                self.logger.info(
                    "[ABORT] Generation resumed, version_after=%d, rid=%s",
                    self.get_version(),
                    req.rid,
                )
            ...
            if stop_reason == "abort":
                n_retries += 1
                self.logger.info(
                    "[ABORT] SGLang returned stop_reason=abort, "
                    "output_tokens=%d, accumulated=%d, version=%d, "
                    "retry=%d, rid=%s",
    ...
    def launch_server(
        self, server_args: dict[str, Any], max_retries: int = 3
    ) -> LocalInfServerInfo:
        """Launch a local inference server with retry on port/startup failures."""
        server_args["host"] = gethostip()
        last_error: Exception | None = None
        for attempt in range(max_retries):
            # sglang 0.5.12 auto-derives grpc_port = http_port + 30000 and
            # rejects anything > 65535 in _handle_deprecated_args, so cap the
            # http port at 35535 to keep the derived grpc port valid.
            server_args["port"] = find_free_ports(1, port_range=(1024, 35535))[0]
            ...
            try:
                ...
                last_error = e
                self.logger.warning(
                    "Launch local server timed out at %s after %ds (attempt %d/%d). "
                ...
                time.sleep(5)  # Wait for port release before retry
        raise TimeoutError(
