...
class RayScheduler(Scheduler):
    def __init__(
        ...
        *,
        ...
        # Mixed layout: unified placement group with per-role bundle assignments
        self._mixed_layout_pg: PlacementGroup | None = None
        self._mixed_layout_assignments: dict[str, list[int]] = {}
        # Per-role PACK PGs for mixed layouts (1 GPU per bundle)
        self._role_layout_pgs: dict[str, PlacementGroup] = {}
    ...
    def setup_mixed_layout(
        ...
        """Pre-allocate a unified STRICT_SPREAD placement group for node pinning.

        Supports two layouts:
        1. Clean split (e.g. d8+d8 on 2×8): each role fills whole nodes.
           Node A: 8 inference, Node B: 8 training.
        2. Mixed (e.g. d10+d6 on 2×8): overflow inference shares a node
           with training. Node A: 8 inference, Node B: 2 inference + 6 training.

        Returns True if a node-pinned layout was activated.
        """
        gen_world_size = allocation_mode.gen.world_size
        train_world_size = allocation_mode.train.world_size
        gen_instance_size = allocation_mode.gen_instance_size
        cross_nodes = gen_instance_size > n_gpus_per_node

        if cross_nodes or n_nodes < 2:
            return False

        remaining_gen_gpus = gen_world_size % n_gpus_per_node
        remaining_train_gpus = train_world_size % n_gpus_per_node
        n_gen_nodes = gen_world_size // n_gpus_per_node

        n_rollout_workers = gen_world_size // gen_instance_size
        workers_per_gen_node = n_gpus_per_node // gen_instance_size

        if remaining_gen_gpus == 0 and remaining_train_gpus == 0:
            # Clean split: each role fills whole nodes.
            # Use two sequential PACK PGs with per-worker (1-GPU) bundles
            # (same approach as mixed layout). PACK fills nodes sequentially:
            # rollout PG fills inference node(s), actor PG fills training node(s).
            # This avoids the large STRICT_SPREAD PG which can time out due to
            # CPU/memory resource constraints.
            n_train_nodes = train_world_size // n_gpus_per_node
            if n_gen_nodes + n_train_nodes > n_nodes:
                return False

            device = ray_resource_type()

            rollout_bundle: dict[str, float] = {
                "CPU": float(rollout_spec.cpu),
                "memory": float(rollout_spec.mem) * 1024**3,
            }
            if device in ("GPU", "NPU"):
                rollout_bundle[device] = 1.0
            rollout_pg = placement_group(
                [rollout_bundle.copy() for _ in range(n_rollout_workers)],
                strategy="PACK",
            )
            try:
                ...
                logger.error(
                    f"Rollout PACK PG timeout ({n_rollout_workers} bundles)"
                )
                raise
            self._placement_groups.append(rollout_pg)

            actor_bundle: dict[str, float] = {
                "CPU": float(actor_spec.cpu),
                "memory": float(actor_spec.mem) * 1024**3,
            }
            if device in ("GPU", "NPU"):
                actor_bundle[device] = 1.0
            actor_pg = placement_group(
                [actor_bundle.copy() for _ in range(train_world_size)],
                strategy="PACK",
            )
            try:
                ...
                logger.error(
                    f"Actor PACK PG timeout ({train_world_size} bundles)"
                )
                raise
            self._placement_groups.append(actor_pg)

            self._role_layout_pgs["rollout"] = rollout_pg
            self._role_layout_pgs["actor"] = actor_pg
            self._mixed_layout_assignments["rollout"] = list(
                range(n_rollout_workers)
            )
            self._mixed_layout_assignments["actor"] = list(
                range(train_world_size)
            )

            logger.info(
                f"Clean split: {n_gen_nodes} inference node(s) + "
                f"{n_train_nodes} training node(s): rollout PACK PG "
                f"({n_rollout_workers} x 1-GPU), actor PACK PG "
                f"({train_world_size} x 1-GPU)"
            )
            return True
        elif (
            remaining_gen_gpus > 0
            and n_gen_nodes >= 1
            and remaining_gen_gpus + train_world_size <= n_gpus_per_node
            and n_gen_nodes + 1 <= n_nodes
        ):
            # Mixed layout: overflow inference + training share a node.
            # Use two sequential PACK PGs with per-worker (1-GPU) bundles:
            #   1. rollout_pg (PACK): fills inference nodes first, overflow to mixed node
            #   2. actor_pg (PACK): gets remaining GPUs on the mixed node
            # Each bundle has 1 GPU → forked workers share the exact same GPU.
            device = ray_resource_type()
            ...
            logger.info(
                f"Mixed layout ({remaining_gen_gpus} overflow inference + "
                f"{train_world_size} training): rollout PACK PG "
                f"({n_rollout_workers} x 1-GPU), actor PACK PG "
                f"({train_world_size} x 1-GPU)"
            )
            return True
        elif gen_world_size + train_world_size <= n_nodes * n_gpus_per_node:
            # Partial split: neither role fills whole nodes, but total fits.
            # E.g. d4+d8 on 2×8: rollout PACK PG packs 4 GPUs on node A,
            # actor PACK PG packs 8 GPUs on node B.
            device = ray_resource_type()
            ...
            logger.info(
                f"Partial split: {gen_world_size} inference + "
                f"{train_world_size} training GPUs across {n_nodes} nodes: "
                f"rollout PACK PG ({n_rollout_workers} x 1-GPU), "
                f"actor PACK PG ({train_world_size} x 1-GPU)"
            )
            return True
        else:
            return False

    def _create_ray_workers_with_layout(
        self, role: str, schedulings: list[SchedulingSpec]
    ) -> tuple[list[RayWorkerInfo], list[str]]:
        """Create workers pinned to pre-allocated layout bundles."""
        # Per-role PACK PGs (mixed layout) or shared STRICT_SPREAD PG (clean split)
        pg = self._role_layout_pgs.get(role, self._mixed_layout_pg)
        assignments = self._mixed_layout_assignments[role]

        if len(schedulings) != len(assignments):
            raise WorkerCreationError(
                role,
                "Mismatch",
                f"Mixed layout expects {len(assignments)} workers "
                f"but got {len(schedulings)}",
            )

        worker_info_list: list[RayWorkerInfo] = []
        worker_ids: list[str] = []

        # Get master IP/port from the first bundle used by this role
        first_bundle = assignments[0]
        master_ip, master_port = get_placement_group_master_ip_and_port(
            pg, placement_group_bundle_index=first_bundle
        )

        for idx, (spec, bundle_idx) in enumerate(zip(schedulings, assignments)):
    ...
    def _create_ray_workers(
        ...
        the same PG/GPU.
        """
        # Use pre-allocated layout PGs if available for this role
        if role in self._role_layout_pgs or (
            self._mixed_layout_pg and role in self._mixed_layout_assignments
        ):
            return self._create_ray_workers_with_layout(role, schedulings)
    ...
    def _create_forked_workers_internal(
        ...
        Each target worker uses num_gpus=0.9, leaving 0.1 per GPU.

        With per-worker PGs (1 GPU each), forked workers request 0.01 GPU.
        The PG boundary forces them onto the target's GPU regardless of fraction.

        With layout PGs (shared bundle with N GPUs), forked workers request
        0.09 GPU to prevent Ray from packing them all onto one GPU.
        Since 0.09 + 0.09 = 0.18 > 0.10 (remaining per GPU), each forked
        worker must land on a different GPU, mirroring the target distribution.
        ...
        for idx, (target_wi, spec) in enumerate(zip(target_workers, schedulings)):
            ...
            # Share GPU with target worker (which uses 0.9, leaving 0.1).
            # With per-worker PGs (1 GPU each), 0.01 is fine — the PG constrains
            # the forked worker to the target's GPU regardless.
            # With layout PGs (clean split only — each role fills whole nodes),
            # use 0.09 so two forks can't share one GPU (0.09+0.09=0.18 > 0.10).
            using_layout_pg = pg is self._mixed_layout_pg
            device = ray_resource_type()
            additional_options = {}
            if spec.gpu > 0:
                fork_gpu = 0.09 if using_layout_pg else 0.01
                if device == "GPU":
                    additional_options = dict(num_gpus=fork_gpu)
                else:
                    additional_options = {"resources": {device: fork_gpu}}
            actor = RayRPCServer.options(
                **additional_options,
    ...
    async def async_call_engine(
        ...
        **kwargs,
        ...
        for attempt in range(1, max_retries + 1):
            try:
                # Pass engine_name to support multiple engines per worker (colocation)
                # Run .remote() in a thread to avoid blocking the asyncio event loop
                # during argument serialization. Without this, each .remote() call
                # blocks for ~5-7s (serializing batch shard data), causing 8 DP workers
                # to start staggered over ~42s instead of simultaneously.
                # Threading helps because the heavy part (tensor memcpy to Ray object
                # store) is C++ code that releases the GIL.
                ref = await asyncio.to_thread(
                    wi.actor.call.remote,
                    method,
                    *args,
