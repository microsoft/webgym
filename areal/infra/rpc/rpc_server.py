...
def _wait_for_worker_ready(host: str, port: int, timeout: float = 600) -> bool:
...
def call_engine_method():
    ...
    try:
        ...
        # Convert tensors to RTensors only for TrainEngine calls.
        # InferenceEngine (rollout) results are returned as plain numpy
        # arrays via Ray RPC's Arrow IPC (zero-copy).  Wrapping them as
        # RTensors causes unnecessary ray.put() into the object store,
        # which fills up and spills to disk (~400s localize regression).
