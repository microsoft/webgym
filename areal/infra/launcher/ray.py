...
def ray_main(config, run_id: int = 0):
    ...
    if allocation_mode.gen_backend == "sglang":
        ...
        # Detect mixed layout: when SGLang doesn't fill whole nodes, co-locate
        # remaining SGLang servers with FSDP training on one node.
        # Example: sglang:d10+d6 on 2×8 GPUs →
        #   Node A (bundle 0): 8 SGLang (full)
        #   Node B (bundle 1): 2 SGLang + 6 FSDP (mixed)
        # This eliminates cross-node gradient sync during FSDP training.
        n_full_sglang_nodes = gen_world_size // n_gpus_per_node
        remaining_sglang_gpus = gen_world_size % n_gpus_per_node
        train_world_size = allocation_mode.train.world_size

        mixed_layout = (
            remaining_sglang_gpus > 0
            and n_full_sglang_nodes >= 1
            and remaining_sglang_gpus + train_world_size <= n_gpus_per_node
            and n_full_sglang_nodes + 1 <= n_nodes
            and not cross_nodes
        )

        if mixed_layout:
            n_sglang_nodes = n_full_sglang_nodes
            n_servers_per_node = n_gpus_per_node
            logger.info(
                f"Mixed layout: {n_full_sglang_nodes} full SGLang node(s) + "
                f"1 mixed node ({remaining_sglang_gpus} SGLang + "
                f"{train_world_size} FSDP)"
            )

            # Create unified placement group with STRICT_SPREAD to guarantee
            # bundles land on different physical nodes.
            gpu_key = "NPU" if is_npu_available else "GPU"
            max_cpus = max(
                rollout_spec.cpu * n_gpus_per_node,
                rollout_spec.cpu * remaining_sglang_gpus
                + actor_spec.cpu * train_world_size,
            )
            max_mem_mb = max(
                rollout_spec.mem * 1024 * n_gpus_per_node,
                rollout_spec.mem * 1024 * remaining_sglang_gpus
                + actor_spec.mem * 1024 * train_world_size,
            )
            unified_bundles = [
                {
                    "CPU": max_cpus,
                    gpu_key: n_gpus_per_node,
                    "memory": max_mem_mb * 1024 * 1024,
                }
            ] * n_nodes
            mixed_pg = ray.util.placement_group(
                bundles=unified_bundles, strategy="STRICT_SPREAD"
            )
            try:
                ray.get(mixed_pg.ready(), timeout=60)
            except ray.exceptions.GetTimeoutError as e:
                logger.error(
                    "Mixed layout placement group timeout. "
                    f"Bundles: {unified_bundles}"
                )
                raise e

            # Pre-register so submit_array reuses this PG for full SGLang nodes
            launcher.placement_groups["llm_server"] = mixed_pg
            mixed_bundle_idx = n_full_sglang_nodes
        else:
            n_sglang_nodes = max(1, gen_world_size // n_gpus_per_node)
        ...
        # Launch full SGLang nodes (all nodes in standard layout, full-only in mixed).
        sglang_cpus_per_task = rollout_spec.cpu * n_gpus_per_node
        sglang_thread_env = get_thread_env_vars(
        ...
        # Launch partial SGLang servers on the mixed node.
        if mixed_layout:
            partial_seed = base_seed + n_full_sglang_nodes * n_servers_per_node
            partial_sglang_cpus = rollout_spec.cpu * remaining_sglang_gpus
            partial_thread_env = get_thread_env_vars(
                cpus_per_task=partial_sglang_cpus,
                existing_env_vars=rollout_spec.env_vars,
            )
            launcher.submit(
                job_name="llm_server_partial",
                file_path=sglang_entry_point,
                func_name=DEFAULT_MAIN_FUNC_NAME,
                args=[
                    sys.argv[1:]
                    + [f"sglang.random_seed={partial_seed}"]
                ],
                gpus=remaining_sglang_gpus,
                cpus=partial_sglang_cpus,
                mem=rollout_spec.mem * 1024 * remaining_sglang_gpus,
                env_vars={
                    **BASE_ENVIRONS,
                    **partial_thread_env,
                    **rollout_spec.env_vars,
                },
                placement_group=mixed_pg,
                bundle_index=mixed_bundle_idx,
            )
            logger.info(
                f"Launched {remaining_sglang_gpus} partial SGLang servers on "
                f"mixed node (bundle {mixed_bundle_idx})"
            )
    ...
    if allocation_mode.type_ != AllocationType.LLM_SERVER_ONLY:
        ...
        if mixed_layout:
            # Mixed layout: launch FSDP trainers on the mixed node's bundle.
            # All training GPUs are on one physical node → no cross-node
            # gradient sync during FSDP forward/backward.
            train_world_size = allocation_mode.train.world_size
            host_ip, port = get_placement_group_master_ip_and_port(
                mixed_pg, mixed_bundle_idx
            )
            logger.info(
                f"FSDP trainer on mixed node: MASTER_ADDR={host_ip}, "
                f"MASTER_PORT={port}, world_size={train_world_size}"
            )
            for rank in range(train_world_size):
                torch_env = {
                    "RANK": str(rank),
                    "WORLD_SIZE": str(train_world_size),
                    "LOCAL_RANK": "0",
                    "MASTER_ADDR": str(host_ip),
                    "MASTER_PORT": str(port),
                }
                launcher.submit(
                    job_name=f"trainer:{rank}",
            ...
            # Standard layout: launch trainers via submit_array with their own PG.
            ...
            def torch_env_hook(
                n_tasks: int, placement_group: PlacementGroup
            ) -> list[dict]:
                host_ip, port = get_placement_group_master_ip_and_port(
                    placement_group
                )
            ...
            launcher.submit_array(
                ...
                env_hook=partial(
                    torch_env_hook, n_trainer_processes
                ),
            )
