...
class StalenessManager:
    ...
    def seed_for_resume(self, version: int) -> None:
        """Seed ``accepted`` so the staleness formula stays balanced after a resume.

        The capacity formula assumes ``accepted ≈ current_version * consumer_batch_size``
        (the steady-state invariant). At process start ``accepted=0`` regardless
        of where training is resuming from, so the staleness brake is effectively
        disabled until enough new rollouts accumulate. We restore the invariant
        explicitly when training resumes mid-run.
        """
        with self.lock:
            self.rollout_stat.accepted = max(0, version) * self.consumer_batch_size
