...
class TrainController:
    ...
    @staticmethod
    def _ensure_rtensors(args: tuple, kwargs: dict) -> tuple[tuple, dict]:
        """Wrap plain-tensor batch dicts as RTensors for DP dispatch.

        When batches come from prepare_batch() without going through RPC
        (e.g. WebGym workflow), they contain plain tensors instead of
        RTensors. Without RTensors, data_parallel_dispatch cannot split
        the batch and replicates everything — causing shape mismatches
        on VLM models where pixel_values get replicated but position
        embeddings are computed per-worker.

        This method finds batch dicts with attention_mask, creates a
        per-sequence RTensor layout, and wraps matching tensors as
        RTensors so dispatch works correctly.
        """
        # Search for batch dict with attention_mask in args.
        # We must check the attention_mask itself (not just any RTensor in the
        # structure) because after actor's compute_logp adds an RTensor result
        # to the batch dict, _find_layout_rtensor would find that RTensor and
        # skip wrapping — leaving the main data tensors as plain tensors that
        # get replicated instead of split during dispatch.
        for i, arg in enumerate(args):
            if isinstance(arg, dict) and "attention_mask" in arg:
                attn_mask = arg["attention_mask"]
                if isinstance(attn_mask, RTensor):
                    continue  # Already wrapped
                if not hasattr(attn_mask, "shape") or attn_mask.ndim < 2:
                    continue
                layout = RTensor.extract_layout(arg, layouts=None, node_addr="")
                if layout is not None:
                    wrapped = RTensor.remotize(arg, layout, node_addr="")
                    args = args[:i] + (wrapped,) + args[i + 1 :]
                    return args, kwargs

        # Search in kwargs
        for key, val in kwargs.items():
            if isinstance(val, dict) and "attention_mask" in val:
                attn_mask = val["attention_mask"]
                if isinstance(attn_mask, RTensor):
                    continue  # Already wrapped
                if not hasattr(attn_mask, "shape") or attn_mask.ndim < 2:
                    continue
                layout = RTensor.extract_layout(val, layouts=None, node_addr="")
                if layout is not None:
                    kwargs = {
                        **kwargs,
                        key: RTensor.remotize(val, layout, node_addr=""),
                    }
                    return args, kwargs
    ...
    @staticmethod
    def _split_multimodal_by_groups(
        mmi: list, group_indices: list[list[int]], total_seqs: int
    ) -> list[list]:
        """Split a flat multi_modal_input list across DP groups.

        When multi_modal_input has more items than sequences (e.g. from
        GroupedRolloutWorkflow concatenating n_samples trajectories),
        data_parallel_dispatch cannot split it automatically because
        len(mmi) != total_shards.  This helper computes per-sequence
        item counts assuming even distribution and slices accordingly.
        """
        n_items = len(mmi)
        if n_items == 0 or total_seqs == 0:
            return [[] for _ in group_indices]

        items_per_seq, remainder = divmod(n_items, total_seqs)
        if remainder != 0:
            # Uneven distribution — cannot split safely, replicate.
            return [mmi for _ in group_indices]

        split = []
        for group_idxs in group_indices:
            group_items = []
            for seq_idx in group_idxs:
                start = seq_idx * items_per_seq
                group_items.extend(mmi[start : start + items_per_seq])
            split.append(group_items)
        return split

    def _dispatch_inputs(self, *args, **kwargs):
        """Split RTensors across DP groups, replicate other args."""
        args, kwargs = self._ensure_rtensors(args, kwargs)
    ...
    def catch_up_lr_scheduler(self, n_steps: int) -> None:
        """Advance the LR scheduler ``n_steps`` times inside each worker (one RPC)."""
        self._custom_function_call("catch_up_lr_scheduler", n_steps=n_steps)
    ...
    def clear_pixel_cache(self):
        """Clear worker-level pixel ref caches between training steps."""
        run_async_task(self._async_clear_pixel_cache)

    async def _async_clear_pixel_cache(self):
        tasks = []
        for idx, worker in enumerate(self.workers):
            tasks.append(
                self.scheduler.async_call_engine(
