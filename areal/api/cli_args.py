...
class OptimizerConfig:
    ...
    lr_scheduler_max_steps: int | None = field(
        default=None,
        metadata={
            "help": "Override total steps for LR scheduler decay. "
            "When set, cosine/linear scheduler decays over this many steps "
            "instead of total_train_epochs * steps_per_epoch.",
        },
    )
...
class SchedulingSpec:
    ...
    image: str = field(
        default="/tmp/areal-test/images/areal-latest.sif",
        metadata={
...
class PPOActorConfig(TrainEngineConfig):
    ...
    ppo_n_epochs: int = field(
        default=1,
        metadata={
            "help": "Inner-loop PPO epochs: number of passes over each rollout "
            "batch (each pass iterates ppo_n_minibatches minibatches). "
            "Default 1 = single pass (current async-RL behavior). "
            "Higher values reuse the same data more aggressively before the "
            "next rollout — beware of stale-policy drift."
        },
    )
    ...
    # Per-step advantage normalization: trajectory-level (default) vs batch-mean
    step_adv_const_length: float | None = field(
        default=None,
        metadata={
            "help": "If set, normalize each step's advantage by this fixed constant "
            "T_const (step_adv = A_g / T_const) instead of the trajectory's own "
            "length T_g. With the default (None) the per-trajectory length is used; "
            "set to a positive constant to make the per-step advantage magnitude "
            "completely independent of T_g and of any other trajectory in the batch. "
            "Typical value: a representative target trajectory length (e.g. 10).",
        },
    )
    ...
    kl_ctl: float = field(
        default=0.1,
        metadata={
            "help": "KL divergence coefficient. Single scalar weighting the "
            "per-token KL term injected into the reward: "
            "kl_rewards = -kl_ctl * KL(pi_old || target). "
            "Set to 0 to fully disable the KL path (the reference model "
            "is then not loaded)."
        },
    )
    ...
    kl_use_prox: bool = field(
        default=False,
        metadata={
            "help": "If True, replace pi_ref with pi_prox (the recomputed "
            "proximal log-prob) as the second distribution in the KL "
            "reward shaping: kl_rewards = -kl_ctl * KL(pi_old || pi_prox). "
            "Only meaningful when use_decoupled_loss=True (otherwise "
            "prox_logp falls back to old_logp and the KL collapses to 0). "
            "When True, the reference model is not loaded."
        },
    )
    use_frpo_loss_term: bool = field(
        default=False,
        metadata={
            "help": "If True, apply FRPO-style advantage modification "
            "instead of KL-via-reward injection. Each microbatch's "
            "advantages become "
            "A_t -= kl_ctl * sum_{t'>=t} clamp(log(pi_theta/pi_prox), "
            "log(1-eps), log(1+eps)). The per-token clamp bounds each "
            "token's contribution to ~eps so the tail-sum variance "
            "scales linearly with sequence length and is not dominated "
            "by outlier tokens. pi_theta is detached (stop-gradient). "
            "Skips compute_advantages' per-token KL reward injection "
            "to avoid double-counting. Requires use_decoupled_loss=True."
        },
    )
    frpo_clip_eps: float = field(
        default=0.2,
        metadata={
            "help": "Per-token clip eps for FRPO advantage modification. "
            "Each token's log(pi_theta/pi_prox) is clamped to "
            "[log(1-eps), log(1+eps)]. Only effective when "
            "use_frpo_loss_term=True."
        },
    )
    frpo_length_norm: bool = field(
        default=False,
        metadata={
            "help": "If True, divide the FRPO tail sum by T (the per-row "
            "total response length, = sum of loss_mask in that row/segment) "
            "before subtracting from advantage: "
            "A_t -= (kl_ctl / T) * sum_{t'>=t} clamp(log(pi_theta/pi_prox)). "
            "Every token in the same row shares the same T, so this is a "
            "per-row scalar rescaling. Equivalent to using a fixed-budget "
            "tail mean times kl_ctl regardless of row length, preventing "
            "long-response rows from accumulating proportionally larger "
            "corrections than short-response rows. Only effective when "
            "use_frpo_loss_term=True."
        },
    )
    use_frpo_weight_clip: bool = field(
        default=False,
        metadata={
            "help": "If True, replace PPO ratio-clipped loss with the "
            "FRPO weight-clipped objective: "
            "L = -mean_t( sg(clip(rho_t * A_t, -eps, +eps)) * log pi_theta_t ) "
            "where rho_t = pi_theta / pi_behave (from old_logp, the "
            "inference-time logp) and eps = frpo_weight_clip_eps. The "
            "ENTIRE weight rho*A is clipped (not just rho), stop-gradient "
            "is applied to the weight so only log pi_theta carries grad. "
            "Mutually exclusive with use_sapo_loss. If "
            "use_frpo_loss_term=True is set alongside, frpo_clip_eps "
            ">= 1.0 will skip the pre-loss FRPO advantage subtraction "
            "(otherwise log(1-eps) underflows)."
        },
    )
    frpo_weight_clip_eps: float = field(
        default=0.2,
        metadata={
            "help": "Clip eps for the weight-clip FRPO loss: "
            "weight = sg(clip(rho_t * A_t, -eps, +eps)). Setting this "
            "very large effectively disables the clip. Only effective "
            "when use_frpo_weight_clip=True."
        },
    )
    ...
    # Critic and actor loss types (combinable)
    # Valid critic_loss: "none", "ppo", "archer"
    # Valid actor_loss: "grpo", "fbc"
    critic_loss: str = field(
        default="none",
        metadata={
            "help": "Critic loss type. 'none': no critic. "
            "'ppo': standard PPO clipped value loss (per-token). "
            "'archer': ArCHer step-level V(s) with MC return targets."
        },
    )
    actor_loss: str = field(
        default="grpo",
        metadata={
            "help": "Actor loss type. 'grpo': standard GRPO/PPO loss "
            "with advantage-based clipping framework "
            "min(rho*A, clip(rho)*A). "
            "'fbc': filtered behavioral cloning on reward>0 trajectories."
        },
    )
    archer_gamma: float = field(
        default=0.95,
        metadata={"help": "Discount factor for ArCHer MC return targets."},
    )
    filter_unchanged_screenshot: bool = field(
        default=False,
        metadata={
            "help": "FBC-only: zero loss_mask for steps whose screenshot is "
            "pixel-identical to the next step's (i.e. the action produced no "
            "visible change, including 'wait'). Filtered steps contribute "
            "neither loss nor denominator. No effect when actor_loss != 'fbc'."
        },
    )

    @property
    def archer_mode(self) -> bool:
        """Backward compat: True when critic uses ArCHer."""
        return self.critic_loss == "archer"

    @property
    def online_fbc(self) -> bool:
        """Backward compat: True when actor uses FBC."""
        return self.actor_loss == "fbc"
...
class PPOCriticConfig(TrainEngineConfig):
    ...
    n_minibatches: int = field(
        default=1, metadata={"help": "Number of minibatches (optimizer steps) per critic update"}
    )
    eps_clip: float = field(
...
class SGLangConfig:
    ...
    # Skip the server warmup phase. sglang 0.5.12+ on Qwen3-VL races its
    # internal warmup HTTP call against server bind, causing one rank to
    # ConnectionRefused and hang the whole rollout. The warmup is purely a
    # readiness probe — it can be safely skipped.
...
class InferenceEngineConfig:
    ...
    max_concurrent_per_worker: None | int = field(
        default=None,
        metadata={
            "help": "Per-worker concurrency limit. When set, each SGLang worker "
            "runs at most this many episodes concurrently. Overrides "
            "max_concurrent_rollouts on the worker side."
        },
    )
    n_samples: int = field(
        default=1,
        metadata={
            "help": "Episodes per task (GRPO group size). Set automatically "
            "from gconfig.n_samples. Used to convert episode-unit config "
            "values to task-unit values for StalenessManager."
        },
    )
    ...
    unique_tasks: bool = field(
        default=False,
        metadata={
            "help": "Skip duplicate dataset tasks within each epoch. "
            "When enabled, each task (identified by its 'task_id' field) "
            "is submitted at most once per pass through the dataset."
        },
    )
...
class RecoverConfig(_Timer):
    ...
    Recovery is always enabled: if a DCP checkpoint exists at the save path,
    it is automatically loaded on startup. Saving is controlled by the caller
    (e.g. triggered by test rollout completion).
    """

    # Kept for backward compatibility with launcher retry logic
    mode: str = field(
        default="auto",
        metadata={"help": "Deprecated. Recovery is always enabled (auto)."},
    )
    retries: int = field(
        default=3,
    ...
    load_only: str = field(
        default="all",
        metadata={
            "help": "Which engines to load from DCP checkpoint. "
            "'all': load actor + critic (default). "
            "'actor': load only actor. "
            "'critic': load only critic. "
            "Useful for resuming critic training with a fresh actor."
        },
    )
