...
def concat_padded_tensors(
    ...
    # Offset image_indices when concatenating episodes with _image_store_keys.
    # Each episode's indices are local (0-based); after concatenating
    # image store key lists, later episodes' indices must be shifted.
    has_image_store = any("_image_store_keys" in td for td in tensor_dicts)
    if has_image_store and "multi_modal_input" in result:
        offset = 0
        mmi_pos = 0
        for td in tensor_dicts:
            bs = get_batch_size(td)
            if offset > 0:
                for idx in range(mmi_pos, mmi_pos + bs):
                    mmi = result["multi_modal_input"][idx]
                    if "image_indices" in mmi:
                        mmi["image_indices"] = [
                            i + offset for i in mmi["image_indices"]
                        ]
            offset += len(td.get("_image_store_keys", []))
            mmi_pos += bs

    # Process each key — use union of all dicts' keys so that keys
    # present in later dicts (e.g. group_id) are not silently dropped.
    all_keys_set: set[str] = set()
    for td in tensor_dicts:
        all_keys_set.update(td.keys())
    all_keys = list(all_keys_set)
    for key in all_keys:
        if is_multi_modal_key(key):
            continue

        # Find first dict that has this key
        first_val = None
        for td in tensor_dicts:
            if key in td:
                first_val = td[key]
        ...
        # Non-tensor values (lists, strings, bools, dicts): concatenate lists,
        # take first value for scalars. WebGym workflows return metadata keys
        # like _screenshot_paths (list), _trajectory_steps (list), _task_info (dict).
        # Some keys (e.g. _evaluation) can be None in some results.
        if not isinstance(first_val, torch.Tensor):
            if any(isinstance(td.get(key), list) for td in tensor_dicts):
                merged = []
                for td in tensor_dicts:
                    val = td.get(key)
        ...
        # Pre-allocate output tensor and slice-copy to avoid O(N) intermediate
        # allocations.  For 4500 sequences × 5 keys the old per-trajectory
        # pad-then-cat loop took ~265 s; this does 1 allocation + N slice copies.
        # Skip dicts that don't have this key (can happen when union of
        # keys is used and some dicts lack certain fields like group_id).
        dicts_with_key = [td for td in tensor_dicts if key in td]
        if not dicts_with_key:
            continue

        if first_val.ndim >= 2:
            total_rows = sum(td[key].shape[0] for td in dicts_with_key)
            extra_dims = first_val.shape[2:]  # e.g. () for [B,S], (H,) for [B,S,H]
            fill = 0 if key == "attention_mask" else pad_value
            out = torch.full(
                (total_rows, max_length, *extra_dims),
                fill,
                dtype=first_val.dtype,
                device=first_val.device,
            )
            row = 0
            for td in dicts_with_key:
                t = td[key]
            ...
            # 1D tensors (rewards, episode_id, group_id, …): just cat
            result[key] = torch.cat([td[key] for td in dicts_with_key], dim=0)
    return result
...
def tensor_container_to(
    ...
    **kwargs,
    ...
    Args:
        skip_keys: If provided, skip these top-level dict keys (leave tensors
            on their current device). Used for lazy/streaming GPU transfer of
            large tensors like pixel_values.
    """
...
class MicroBatchList:
    ...
    def to(self, *args, stream_pixel_values: bool = False, **kwargs):
        # When stream_pixel_values=True, skip moving pixel_values to GPU
        # during the bulk transfer. They will be moved per-microbatch in
        # forward_backward_batch() to avoid holding all images on GPU at once.
        _skip = {"pixel_values"} if stream_pixel_values else None
        mbs = [
            tensor_container_to(mb, *args, skip_keys=_skip, **kwargs)
            for mb in self.mbs
        ]
        data = tensor_container_to(self.data, *args, skip_keys=_skip, **kwargs)
        padded_mbs = None
    ...
    def __call__(
        ...
        if self.mean_level == "batch":
            ...
            if group_ids is not None:
                # GRPO: group by explicit group_ids (same task's episodes)
                unique_gids = torch.unique(group_ids)
                for gid in unique_gids:
                    mask = group_ids == gid
                    xx = x[mask]
                    m = loss_mask[mask] if loss_mask is not None else None
                    ...
                    mean[mask] = group_mean.expand_as(xx).to(mean.dtype)
            else:
                # Positional grouping (FBC path)
        ...
        if self.std_level == "batch":
            ...
            if group_ids is not None:
                # GRPO: group by explicit group_ids
                unique_gids = torch.unique(group_ids)
                for gid in unique_gids:
                    mask = group_ids == gid
                    xx = x[mask]
                    m = loss_mask[mask] if loss_mask is not None else None
                    group_mean_slice = mean[mask]
                    group_std = self._compute_std(
                        xx, m, group_mean_slice,
                        unbiased=self.std_unbiased,
                        high_precision=high_precision,
                        all_reduce=False,
                        reduce_group=None,
                    )
                    std[mask] = group_std.expand_as(xx).to(std.dtype)
            else:
                # Positional grouping (FBC path)
                for i in range(0, bs // self.group_size):
                    s = slice(i * self.group_size, (i + 1) * self.group_size)
