...
def ppo_actor_loss_fn(
    ...
    if use_frpo_weight_clip:
        # FRPO weight-clip: clip the WHOLE term sg(rho * A) instead of
        # clipping rho alone. Gradient flows only through log pi_theta.
        # Per the spec, rho is computed against the behavior policy
        # (inference-time old_logp), not the proximal recomputed logp, so
        # no additional behav_imp_weight correction is applied below.
        rho_behave = torch.where(
            loss_mask, torch.exp(logprobs - old_logprobs), 0.0
        )
        raw_weight = rho_behave.detach() * advantages
        weight = raw_weight.clamp(
            min=-frpo_weight_clip_eps, max=frpo_weight_clip_eps,
        )
        pg_loss = -weight * logprobs
        # `clip_mask` flags tokens whose unclipped weight was pushed onto
        # the [-eps, +eps] boundary (engagement rate diagnostic).
        clip_mask = raw_weight != weight
    ...
    # FRPO weight-clip already uses rho = pi_theta / pi_behave directly via
    # old_logp; the additional behav_imp_weight correction (pi_prox/pi_behave)
    # would double-count, so we skip it.
    if not use_frpo_weight_clip:
