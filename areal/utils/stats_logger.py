...
class StatsLogger:
    ...
    def init(self):
        ...
        if suffix == "timestamp":
            # Persist the timestamp suffix per (experiment, trial) so relaunches
            # produce the same wandb run ID and resume("allow") attaches to the
            # original run instead of creating a fresh one each restart.
            suffix_dir = os.path.join(self.config.fileroot, "wandb_id")
            suffix_file = os.path.join(
                suffix_dir,
                f"{self.config.experiment_name}__{self.config.trial_name}",
            )
        ...
        wandb.init(
            ...
            or f"{self.config.experiment_name}/{self.config.trial_name}",
