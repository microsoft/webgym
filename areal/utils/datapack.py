...
def balanced_greedy_partition(nums: list[int], K: int) -> list[list[int]]:
    ...
    # When n is not divisible by K, the first (n % K) groups get one extra item.
    base_size = n // K
    remainder = n % K
    ...
    max_counts = [base_size + (1 if i < remainder else 0) for i in range(K)]
    ...
    for idx in sorted_indices:
        ...
        min_sum = float("inf")
        for i in range(K):
            if counts[i] < max_counts[i] and sums[i] < min_sum:
                min_sum = sums[i]
                chosen_group = i
